...
export async function backup() {
  const filename = `gilt-studio.${dayjs().format('YYYYMMDDHHmm')}.zip`
...
async function handleData(data: Record<string, any>) {
  if (data.version === 1) {
    ...
    await localStorage.setItem('persist:gilt-studio', data.localStorage['persist:gilt-studio'])
  ...
  if (data.version >= 2) {
    localStorage.setItem('persist:gilt-studio', data.localStorage['persist:gilt-studio'])
